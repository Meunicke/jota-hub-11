...
function TitanHub:BuildReachSection()
    ...
    -- Show Sphere Toggle - CORRIGIDO: criar container manualmente
    local sphereContainer = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -20, 0, 40),
        Position = UDim2.new(0, 0, 0, 140)
    })
    sphereContainer.Parent = section

    local sphereLabel = Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    sphereLabel.Parent = sphereContainer

    local sphereSwitchBg = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(50, 50, 60),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 50, 0, 26),
        Position = UDim2.new(1, -50, 0.5, -13)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = sphereSwitchBg

    local sphereStroke = Create("UIStroke", {
        Color = CADU_CONFIG.showReachSphere and self.Theme.Success or Color3.fromRGB(80, 80, 90),
        Thickness = 2,
        Transparency = 0.5
    })
    sphereStroke.Parent = sphereSwitchBg
    sphereSwitchBg.Parent = sphereContainer

    local sphereCircle = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 20, 0, 20),
        Position = CADU_CONFIG.showReachSphere and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = sphereCircle
    sphereCircle.Parent = sphereSwitchBg

    local sphereClick = Create("TextButton", {
        BackgroundTransparency = 1,
        Text = "",
        Size = UDim2.new(1, 0, 1, 0)
    })
    sphereClick.Parent = sphereContainer

    local sphereState = CADU_CONFIG.showReachSphere
    sphereClick.MouseButton1Click:Connect(function()
        sphereState = not sphereState
        CADU_CONFIG.showReachSphere = sphereState

        Tween(sphereCircle, 0.3, {Position = sphereState and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)})
        Tween(sphereSwitchBg, 0.3, {BackgroundColor3 = sphereState and Color3.fromRGB(0, 100, 80) or Color3.fromRGB(50, 50, 60)})
        Tween(sphereStroke, 0.3, {Color = sphereState and self.Theme.Success or Color3.fromRGB(80, 80, 90)})

        notify("Reach", "Esfera " .. (sphereState and "ativada" or "desativada"), 2)
    end)
end
...
function TitanHub:BuildControlsSection()
    ...
    -- Auto Touch Toggle
    local autoTouchContainer = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -20, 0, 40),
        Position = UDim2.new(0, 0, 0, 35)
    })
    autoTouchContainer.Parent = section

    local autoTouchLabel = Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    autoTouchLabel.Parent = autoTouchContainer

    local autoTouchBg = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(50, 50, 60),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 50, 0, 26),
        Position = UDim2.new(1, -50, 0.5, -13)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = autoTouchBg

    local autoTouchStroke = Create("UIStroke", {
        Color = CADU_CONFIG.autoTouch and self.Theme.Success or Color3.fromRGB(80, 80, 90),
        Thickness = 2,
        Transparency = 0.5
    })
    autoTouchStroke.Parent = autoTouchBg
    autoTouchBg.Parent = autoTouchContainer

    local autoTouchCircle = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 20, 0, 20),
        Position = CADU_CONFIG.autoTouch and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = autoTouchCircle
    autoTouchCircle.Parent = autoTouchBg

    local autoTouchClick = Create("TextButton", {
        BackgroundTransparency = 1,
        Text = "",
        Size = UDim2.new(1, 0, 1, 0)
    })
    autoTouchClick.Parent = autoTouchContainer

    local autoTouchState = CADU_CONFIG.autoTouch
    autoTouchClick.MouseButton1Click:Connect(function()
        autoTouchState = not autoTouchState
        CADU_CONFIG.autoTouch = autoTouchState

        Tween(autoTouchCircle, 0.3, {Position = autoTouchState and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)})
        Tween(autoTouchBg, 0.3, {BackgroundColor3 = autoTouchState and Color3.fromRGB(0, 100, 80) or Color3.fromRGB(50, 50, 60)})
        Tween(autoTouchStroke, 0.3, {Color = autoTouchState and self.Theme.Success or Color3.fromRGB(80, 80, 90)})

        notify("Auto Touch", autoTouchState and "Ativado" or "Desativado", 2)
    end)

    -- Full Body Toggle
    local fullBodyContainer = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -20, 0, 40),
        Position = UDim2.new(0, 0, 0, 80)
    })
    fullBodyContainer.Parent = section

    local fullBodyLabel = Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    fullBodyLabel.Parent = fullBodyContainer

    local fullBodyBg = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(50, 50, 60),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 50, 0, 26),
        Position = UDim2.new(1, -50, 0.5, -13)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = fullBodyBg

    local fullBodyStroke = Create("UIStroke", {
        Color = CADU_CONFIG.fullBodyTouch and self.Theme.Success or Color3.fromRGB(80, 80, 90),
        Thickness = 2,
        Transparency = 0.5
    })
    fullBodyStroke.Parent = fullBodyBg
    fullBodyBg.Parent = fullBodyContainer

    local fullBodyCircle = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 20, 0, 20),
        Position = CADU_CONFIG.fullBodyTouch and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = fullBodyCircle
    fullBodyCircle.Parent = fullBodyBg

    local fullBodyClick = Create("TextButton", {
        BackgroundTransparency = 1,
        Text = "",
        Size = UDim2.new(1, 0, 1, 0)
    })
    fullBodyClick.Parent = fullBodyContainer

    local fullBodyState = CADU_CONFIG.fullBodyTouch
    fullBodyClick.MouseButton1Click:Connect(function()
        fullBodyState = not fullBodyState
        CADU_CONFIG.fullBodyTouch = fullBodyState

        Tween(fullBodyCircle, 0.3, {Position = fullBodyState and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)})
        Tween(fullBodyBg, 0.3, {BackgroundColor3 = fullBodyState and Color3.fromRGB(0, 100, 80) or Color3.fromRGB(50, 50, 60)})
        Tween(fullBodyStroke, 0.3, {Color = fullBodyState and self.Theme.Success or Color3.fromRGB(80, 80, 90)})

        notify("Full Body", fullBodyState and "Ativado" or "Desativado", 2)
    end)

    -- Double Touch Toggle
    local doubleTouchContainer = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -20, 0, 40),
        Position = UDim2.new(0, 0, 0, 125)
    })
    doubleTouchContainer.Parent = section

    local doubleTouchLabel = Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    doubleTouchLabel.Parent = doubleTouchContainer

    local doubleTouchBg = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(50, 50, 60),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 50, 0, 26),
        Position = UDim2.new(1, -50, 0.5, -13)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = doubleTouchBg

    local doubleTouchStroke = Create("UIStroke", {
        Color = CADU_CONFIG.autoSecondTouch and self.Theme.Success or Color3.fromRGB(80, 80, 90),
        Thickness = 2,
        Transparency = 0.5
    })
    doubleTouchStroke.Parent = doubleTouchBg
    doubleTouchBg.Parent = doubleTouchContainer

    local doubleTouchCircle = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 20, 0, 20),
        Position = CADU_CONFIG.autoSecondTouch and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = doubleTouchCircle
    doubleTouchCircle.Parent = doubleTouchBg

    local doubleTouchClick = Create("TextButton", {
        BackgroundTransparency = 1,
        Text = "",
        Size = UDim2.new(1, 0, 1, 0)
    })
    doubleTouchClick.Parent = doubleTouchContainer

    local doubleTouchState = CADU_CONFIG.autoSecondTouch
    doubleTouchClick.MouseButton1Click:Connect(function()
        doubleTouchState = not doubleTouchState
        CADU_CONFIG.autoSecondTouch = doubleTouchState

        Tween(doubleTouchCircle, 0.3, {Position = doubleTouchState and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)})
        Tween(doubleTouchBg, 0.3, {BackgroundColor3 = doubleTouchState and Color3.fromRGB(0, 100, 80) or Color3.fromRGB(50, 50, 60)})
        Tween(doubleTouchStroke, 0.3, {Color = doubleTouchState and self.Theme.Success or Color3.fromRGB(80, 80, 90)})

        notify("Double Touch", doubleTouchState and "Ativado" or "Desativado", 2)
    end)
end
...
function TitanHub:BuildSkillsSection()
    ...
    -- Auto Skills Toggle
    local skillsContainer = Create("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -20, 0, 40),
        Position = UDim2.new(0, 0, 0, 35)
    })
    skillsContainer.Parent = section

    local skillsLabel = Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    skillsLabel.Parent = skillsContainer

    local skillsBg = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(50, 50, 60),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 50, 0, 26),
        Position = UDim2.new(1, -50, 0.5, -13)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = skillsBg

    local skillsStroke = Create("UIStroke", {
        Color = CADU_CONFIG.autoSkills and self.Theme.Success or Color3.fromRGB(80, 80, 90),
        Thickness = 2,
        Transparency = 0.5
    })
    skillsStroke.Parent = skillsBg
    skillsBg.Parent = skillsContainer

    local skillsCircle = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(255, 255, 255),
        BorderSizePixel = 0,
        Size = UDim2.new(0, 20, 0, 20),
        Position = CADU_CONFIG.autoSkills and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)
    })
    Create("UICorner", {CornerRadius = UDim.new(1, 0)}).Parent = skillsCircle
    skillsCircle.Parent = skillsBg

    local skillsClick = Create("TextButton", {
        BackgroundTransparency = 1,
        Text = "",
        Size = UDim2.new(1, 0, 1, 0)
    })
    skillsClick.Parent = skillsContainer

    local skillsState = CADU_CONFIG.autoSkills
    skillsClick.MouseButton1Click:Connect(function()
        skillsState = not skillsState
        CADU_CONFIG.autoSkills = skillsState

        Tween(skillsCircle, 0.3, {Position = skillsState and UDim2.new(1, -24, 0.5, -10) or UDim2.new(0, 4, 0.5, -10)})
        Tween(skillsBg, 0.3, {BackgroundColor3 = skillsState and Color3.fromRGB(0, 100, 80) or Color3.fromRGB(50, 50, 60)})
        Tween(skillsStroke, 0.3, {Color = skillsState and self.Theme.Success or Color3.fromRGB(80, 80, 90)})

        notify("Auto Skills", skillsState and "Ativado" or "Desativado", 2)
    end)
